...
class DataTransformation:
    ...
    def load_image_name_set(self, filename) -> set:
        logger.info("Entered the load_image_name_set methos of Data Transformation class")
        try:
            doc = self.data_transformation_config.UTILS.read_txt_file(filename)
            dataset = list()
            ...
            logger.info("Exited the load_image_name_set methos of Data Transformation class")
            return set(dataset)
    ...
    def prepare_descriptions(self, filename, dataset) -> dict:
        logger.info("Entered the prepare_descriptions method of Data Transformation class")
        try:
            # load document
            doc = self.data_transformation_config.UTILS.read_txt_file(filename)
            descriptions = dict()
            ...
            logger.info("Exited the prepare_descriptions method of Data Transformation class")
            return descriptions
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        ...
        try:
            ...
            train_token_file = self.data_transformation_config.UTILS.read_txt_file(train_token_file_path)
            ...
            test_token_file = self.data_transformation_config.UTILS.read_txt_file(test_token_file_path)
            ...
            cleaned_train_desc_path = self.data_transformation_config.UTILS.save_descriptions(descriptions=cleaned_train_mapping, 
                                                                                                filename=self.data_transformation_config.CLEANED_TRAIN_DESC_PATH)
            logger.info(f"saved the train descriptions to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.CLEANED_TRAIN_DESC_PATH)}")
            cleaned_test_desc_path = self.data_transformation_config.UTILS.save_descriptions(descriptions=cleaned_test_mapping, 
                                                                                                filename=self.data_transformation_config.CLEANED_TEST_DESC_PATH)
            logger.info(f"saved the test descriptions to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.CLEANED_TEST_DESC_PATH)}")
            ...
            train_image_path = self.data_transformation_config.UTILS.save_txt_file(output_file_path=self.data_transformation_config.TRAIN_IMAGE_WITH_PATH, 
                                                                                                    data=train_images) 
            logger.info(f"Saved train images to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.TRAIN_IMAGE_WITH_PATH)}")
            test_image_path = self.data_transformation_config.UTILS.save_txt_file(output_file_path=self.data_transformation_config.TEST_IMAGE_WITH_PATH, 
                                                                                                    data=test_images)
            logger.info(f"Saved test images to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.TEST_IMAGE_WITH_PATH)}")

            # Preparing the cleaned descriptions.
            train_img_with_cleaned_desc = self.prepare_descriptions(filename=self.data_transformation_config.CLEANED_TRAIN_DESC_PATH, dataset=train_image_txt_names)
            test_img_with_cleaned_desc = self.prepare_descriptions(filename=self.data_transformation_config.CLEANED_TEST_DESC_PATH, dataset=test_image_txt_names)
            logger.info("Prepared the train and test descriptions")

            # Saving the cleaned descriptions to the artifacts directory.
            train_img_with_cleaned_desc_path = self.data_transformation_config.UTILS.save_txt_file(output_file_path=self.data_transformation_config.TRAIN_IMAGE_WITH_CLEANED_DESC_PATH, 
                                                                                                    data=train_img_with_cleaned_desc)
            logger.info(f"Saved the train descriptions with image names to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.TRAIN_IMAGE_WITH_CLEANED_DESC_PATH)}")
            test_img_with_cleaned_desc_path = self.data_transformation_config.UTILS.save_txt_file(output_file_path=self.data_transformation_config.TEST_IMAGE_WITH_CLEANED_DESC_PATH, 
                                                                                                    data=test_img_with_cleaned_desc)
            logger.info(f"Saved the test descriptions with image names to the artifacts directory. File name - {os.path.basename(self.data_transformation_config.TEST_IMAGE_WITH_CLEANED_DESC_PATH)}")

            data_transformation_artifacts = DataTransformationArtifacts(cleaned_train_desc_path=cleaned_train_desc_path,
